...
begin
  ...
  {$IFDEF _TRACE_CALLS_}
  try try {$R+}
  LogEnterProc('getSQLDriverODBCW', ['sVendorLib =', StrPas(sVendorLib), 'sResourceFile =', sResourceFile, 'Obj =', Pointer(Obj)] ); {$IFDEF RANGECHECKS_OFF}{$R-}{$ENDIF}
  {$ENDIF}
  Pointer(Obj) := nil;
...
begin
  ...
  {$IFDEF _TRACE_CALLS_}
  try try {$R+}
  LogEnterProc('getSQLDriverODBCWA', ['sVendorLib =', StrPas(sVendorLib), 'sResourceFile =', sResourceFile, 'Obj =', Pointer(Obj)] ); {$IFDEF RANGECHECKS_OFF}{$R-}{$ENDIF}
  {$ENDIF}
  Pointer(Obj) := nil;
...
begin
  Result := StrLComp(ADbxCmdText, PAnsiChar(AnsiString(DBX4_CMD_PREFIX)), Length(DBX4_CMD_PREFIX)) = 0;
end;
...
begin
  if AUnicode then
    ...
    Result := StrLComp(PAnsiChar(ADbxCmdText), PAnsiChar(AnsiString(DBX4_CMD_PREFIX)), Length(DBX4_CMD_PREFIX)) = 0;
end;
