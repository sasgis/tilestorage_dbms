...
const
  ...
  // формат для показа в списке версий и для передачи даты через http
  // этот формат обязан парситься юнитом u_Tile_Parser
...
begin
  // если одна таблица - значит не делимся - вернём нули
  if UseSingleTable(AXYMaskWidth, Zoom) then begin
    Result := nil;
  end else begin
    // делимся
