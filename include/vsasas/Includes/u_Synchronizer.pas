...
{$if defined(DEBUG_GLOBAL_LOCKS)}
procedure DoDebugGlobalLocks(const ASync: TSynchronizerAbstract; const AProcedure, AEvent: AnsiString);
const
  c_SEP = ', ' + Chr(VK_TAB);
var
  VText: AnsiString;
begin
  ...
  VText := ASync.ClassName + '=$'+ IntToHex(Integer(Pointer(ASync)), 8) + c_SEP +
           AProcedure + c_SEP +
           AEvent + c_SEP +
           ASync.FCreatorClassName + c_SEP +
