...
const
  ...
  // decompress error (not decompressed)
  ETS_RESULT_DECOMPRESS_FAILED  = 24;
  // decompress error (decompressed but size mismatch)
  ETS_RESULT_DECOMPRESS_SIZE    = 25;
  // decompress error (decompressed but empty)
  ETS_RESULT_DECOMPRESS_EMPTY   = 26;
...
const
  ...
  ETS_ROI_DONT_SAVE_SAME_PREV  = $00000200; // dont save the same (check with prev version)
  ETS_ROI_KEEP_EXISTING        = $00000400; // keep existing tile or tne (for Save and SetVersion)
...
type
  ...
  // tile info for replace version
  PETS_SET_TILE_VERSION_IN = ^TETS_SET_TILE_VERSION_IN;
  TETS_SET_TILE_VERSION_IN = packed record
    XYZ: PTILE_ID_XYZ;
    szOldVersion: Pointer; // Source Version (PAnsiChar or PWideChar)
    szNewVersion: Pointer; // Target Version (PAnsiChar or PWideChar)
    dwOptionsIn: LongWord; // ETS_ROI_* constants
  end;
