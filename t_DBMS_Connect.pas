...
const
  ...
  // если указан DSN (после DSN:) - используются параметры текущей секции (в том числе пароль)
  ...
  // для большой скорости работы (операции умножения и деления на 2) задаётся тайлами как прямоугольник
  // задаётся зум и номера тайлов, нижняя граница включается, верхняя - исключается
  // например Rect:z8\x84\y36\x85\y37 задаёт прямоугольник в 1 тайл на 8 зуме
